...
$qantity_query=mysqli_query($con,"SELECT * FROM `price_list_service` WHERE product_id='".$data[0]."' order by id ASC LIMIT 1");
...
if(check_array($id,$qty['id'],$cooks))
{
    ...
    array_push($cooks, array("id" => $_POST['id'], "Quantity" => $qty['id'], "description" => $data['product_name'], "price" => $data['product_price'], "img_path" => $base_url."/product/".$data['product_image']));
